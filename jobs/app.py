...
def review(employer_id):
    if request.method == "POST":
        ...
        date = datetime.utc_now().strftime('%m/%d/%Y')
        execute_sql('INSERT INTO review (review, rating, title, date, status, employer_id) VALUES (?, ?, ?, ?, ?, ?)', (reviews, rating, title, date, status, employer_id), commit=True)
